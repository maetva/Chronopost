...
class DestinatairesDpd extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd[]
     */
    protected ?array $destinataireDpd = null;
    /**
     * Constructor method for destinatairesDpd
     * @uses DestinatairesDpd::setDestinataireDpd()
     * @param \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd[] $destinataireDpd
     */
    ...
    public static function validateDestinataireDpdForArrayConstraintsFromSetDestinataireDpd(?array $values = []): string
    {
        ...
        foreach ($values as $destinatairesDpdDestinataireDpdItem) {
            ...
            if (!$destinatairesDpdDestinataireDpdItem instanceof \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The destinataireDpd property can only contain items of type \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd[] $destinataireDpd
     * @return \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinatairesDpd
     */
     ...
     * @throws InvalidArgumentException
     * @param \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd $item
     * @return \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinatairesDpd
     */
    public function addToDestinataireDpd(\Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd) {
            throw new InvalidArgumentException(sprintf('The destinataireDpd property can only contain items of type \Drupal\commerce_chronopost\Endpoint\ShippingServiceWS\StructType\DestinataireDpd, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
