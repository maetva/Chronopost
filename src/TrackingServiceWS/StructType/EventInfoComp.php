...
class EventInfoComp extends Event
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp[]
     */
    protected ?array $infoCompList = null;
    /**
     * Constructor method for eventInfoComp
     * @uses EventInfoComp::setInfoCompList()
     * @param \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp[] $infoCompList
     */
     ...
     * @return \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp[]
     */
    ...
    public static function validateInfoCompListForArrayConstraintsFromSetInfoCompList(?array $values = []): string
    {
        ...
        foreach ($values as $eventInfoCompInfoCompListItem) {
            ...
            if (!$eventInfoCompInfoCompListItem instanceof \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The infoCompList property can only contain items of type \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp $item
     * @return \Maetva\Chronopost\TrackingServiceWS\StructType\EventInfoComp
     */
    public function addToInfoCompList(\Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp) {
            throw new InvalidArgumentException(sprintf('The infoCompList property can only contain items of type \Maetva\Chronopost\TrackingServiceWS\StructType\InfoComp, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
