...
class ListEvents extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \Maetva\Chronopost\TrackingServiceWS\StructType\Event[]
     */
     ...
     * @param \Maetva\Chronopost\TrackingServiceWS\StructType\Event[] $events
     ...
     * @return \Maetva\Chronopost\TrackingServiceWS\StructType\Event[]
     */
    ...
    public static function validateEventsForArrayConstraintsFromSetEvents(?array $values = []): string
    {
        ...
        foreach ($values as $listEventsEventsItem) {
            ...
            if (!$listEventsEventsItem instanceof \Maetva\Chronopost\TrackingServiceWS\StructType\Event) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The events property can only contain items of type \Maetva\Chronopost\TrackingServiceWS\StructType\Event, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Maetva\Chronopost\TrackingServiceWS\StructType\Event $item
     * @return \Maetva\Chronopost\TrackingServiceWS\StructType\ListEvents
     */
    public function addToEvents(\Maetva\Chronopost\TrackingServiceWS\StructType\Event $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Maetva\Chronopost\TrackingServiceWS\StructType\Event) {
            throw new InvalidArgumentException(sprintf('The events property can only contain items of type \Maetva\Chronopost\TrackingServiceWS\StructType\Event, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->events[] = $item;
